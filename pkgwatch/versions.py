...
:link.versionlink, :visited.versionlink
{
   color: black;
   text-decoration: none;
}
...
tr.newboth
{
    background-color: #ffa17e; /* orangered, #ff4500 */
}
tr.newupstream
{
...
tr.syncnewupstream
{
    background-color: #ffd280; /* orange, #ffa500 */
}
tr.newdebian
{
    background-color: #ffff80; /* yellow, #ffff00 */
}
tr.newunstable
{
    background-color: #ddfd7f;
}
tr.uptodate
{
    background-color: #bcfc7e; /* lawngreen, #7cfc00 */
}
tr.untracked
{
...
td
{
    ...
    html_out = replace_template_var(html_template, "extra_css", html)
...
'''
    ...
    for source in sources:
        ...
        html += '&nbsp;(<a href="http://packages.qa.debian.org/%c/%s.html">PTS</a>)</td>\n' % (package.source[0], package.source)
...
"""
    ...
    html_out = replace_template_var(html_out, "content", html)

    html = '<p>\n'
    html += 'Last updated: %s, took %d minutes, %d seconds<br>\n' % (time.strftime('%A %B %d %Y %H:%M:%S %z'), n_minutes, n_seconds)
    html += 'These pages are generated by pkgcheck (part of Rapidumo) (<a href="http://gitorious.org/tanglu/rapidumo">sources</a>). It contains code originally written for Ubuntu.\n'
    html += '</p>\n'

    html_out = replace_template_var(html_out, "extra_footer", html)

    return html_out

def write_page (name, tags = [], invert = False):
    html = generate_page (tags, invert)
    f = open(name, 'w')
    f.write(html)
    f.close()
    debug('Wrote %s' % name)
